...
def graphrm(id_param, psd_list_param, tid_param):
    ...
    with print_lock:
        sys.stdout.write(f'\r\r{W}[{G}={W}]{R}~{W}[{G}CYBER-0071 HACKER TM {W}-{G}M1{W}]>~[{G}{lop}{W}]>~<[{G}{tid_param}{W}]>~[{G}OK{R}•{G}{len(ok)}{R}/{H}{len(cp)}{W}] ')
        sys.stdout.flush()
...
def rmenu1():
    ...
    if '2' in rcd: # Contact Admin option was chosen in Main
        ...
        os.system('xdg-open https://github.com/darkcyber0071-dev')
        time.sleep(3)
    ...
    with ThreadPoolExecutor(max_workers=60) as tonxoys:
        ...
        print(f'{G}┣━{W}[{G}+{W}]{B} \033[0;35mCREATOR  BY{R} :{S} EHC .  EV AN ') # Example, original had no name here
